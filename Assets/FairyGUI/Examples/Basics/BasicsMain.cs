...
public class BasicsMain : MonoBehaviour
{
	...
	private void runDemo(EventContext context)
	{
		...
		//按钮的名称 btn_Button/btn_Image/btn_Graph......
		//获取到类型,从第4位读取Button/Image/Graph.....
		//(GObject)(context.sender) 上下文事件的发送者
		string type = ((GObject)(context.sender)).name.Substring(4);
		GComponent obj;
		//每个按钮都会跳转到一个组件(Demo_...) , 如果字典中不存在则创建
		...
		//demo容器组件移除所有元件,添加新元件.  显示控制器跳转到1界面,返回按钮设置为
